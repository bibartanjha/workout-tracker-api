...
def get_recent_workout(exercise: str, num_workouts: int, db: Session = Depends(get_db)):
    row = (
        db.query(WorkoutRecord)
        .filter(WorkoutRecord.exercise == exercise)
        .order_by(WorkoutRecord.Date.desc())
        .limit(num_workouts)
    )
    if row:
        return db_row_to_workout(row_to_dict(row))
    else:
